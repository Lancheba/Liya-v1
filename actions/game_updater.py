...
def _game_updater_impl(parameters: dict, player=None, speak=None) -> str:
    p         = parameters or {}
...
# Substrings that, if present ANYWHERE in the combined result text, mean at
# least part of the request did not succeed. This module composes results
# from multiple sub-calls (e.g. "Steam: ... | Epic: ..."), so a single
# failure fragment must not be masked by a successful fragment elsewhere —
# conservative bias toward reporting failure, since a false "success" is the
# more expensive mistake (see agent/executor.py's false-success bug history).
_FAILURE_SUBSTRINGS = (
    "not installed", "no games found", "no steam games found",
    "not found", "could not start steam", "could not find",
    "could not detect drives", "please specify", "please select",
    "please click install manually", "install failed", "scheduling failed",
    "cancel failed", "epic launch failed", "epic update failed",
    "not natively supported", "no platforms found",
    "unknown action", "nothing to do",
)

# These are legitimate, complete answers to a read-only status query — an
# empty result isn't a failure to accomplish anything, unlike "not found"
# for an install/update action. Excluded from the failure list above.
_STATUS_QUERY_ACTIONS = frozenset({"schedule_status", "download_status", "list"})


def _is_failure_text(text: str, action: str = "") -> bool:
    low = text.lower()
    if action in _STATUS_QUERY_ACTIONS and (
        "no scheduled game update found" in low
        or "no active downloads or pending updates" in low
        or "no games found" in low
        or "no steam games found" in low
        or "no platforms found" in low
    ):
        # An empty result is a complete, legitimate answer to a status/list
        # query — not a failure to accomplish anything.
        return False
    return any(s in low for s in _FAILURE_SUBSTRINGS)


def game_updater(parameters: dict, player=None, speak=None):
    action      = (parameters or {}).get("action", "update").lower().strip()
    result_text = _game_updater_impl(parameters, player=player, speak=speak)
    if _is_failure_text(result_text, action=action):
        return fail(result_text)
...
if __name__ == "__main__":
    if "--scheduled" in sys.argv:
        ...
        print(f"[GameUpdater] ✅ {result['message']}")
